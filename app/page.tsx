    ...
    function validarForm(): boolean {
        ...
        for (let campo of campos) {
            if (campo.obrigatorio && !formData[campo.nome].trim()) {
                tempErrors[campo.nome] =
                    `Você precisa prencher o ${campo.etiqueta}`;
            }
        }
        ...
        <section className="flex flex-col gap-1">
            {cifras.map((cifra) => (
                <div key={cifra.id} className="flex w-full">
                    <button className="rounded-md bg-gray-900 p-2">
                        <Icone
                            x={40}
                            y={40}
                    ...
                    <a
                        href={cifra.link}
                        target="_blank"
                        className="grow flex flex-col justify-start ml-1 rounded-md bg-gray-900 p-2"
                    >
                        <span className="text-3xl">{cifra.nome}</span>
                        <span className="font-medium">{cifra.autor}</span>
                    </a>
                </div>
            ))}
        </section>
...
export default function Main() {
    ...
    return (
        <main className="flex flex-col m-1 justify-center rounded-md gap-4">
