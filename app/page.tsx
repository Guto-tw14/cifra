...
function ListaCifras({
    ...
    function destacarPesquisa(texto: string, pesquisa: string) {
        ...
        while (true) {
            const indice = textoMinusculo.indexOf(pesquisaMinuscula, inicio);
            if (indice == -1) {
                elementos.push(texto.slice(inicio));
                break;
            }
                ...
                <div key={cifra.id} className="flex w-full">
                    ...
                    >
                        <span className="text-xl">
                            {destacarPesquisa(cifra.nome, pesquisa)}
                        </span>
                        <span className="text-sm text-text-subtle">
                            {destacarPesquisa(cifra.autor, pesquisa)}
                        </span>
                    </a>
                </div>
            ))}
